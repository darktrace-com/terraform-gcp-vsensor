...
| <a name="input_mirrored_cidr_ranges"></a> [mirrored\_cidr\_ranges](#input\_mirrored\_cidr\_ranges) | IP CIDR ranges that apply as a filter on the source (ingress) or destination (egress) IP in the IP header.<br>  Only IPv4 is supported.<br>  Use 0.0.0.0/0 to allow all IP ranges.<br>  Example: ["10.0.0.0/24", "10.0.1.0/24"].<br>  The default value is "0.0.0.0/0". | `list(string)` | <pre>[<br>  "0.0.0.0/0"<br>]</pre> | no |
